...
public actor Downloader {
    ...
    public func download(
        ...
        if let sha1 {
            ...
            guard actual == sha1 else {
                ...
                throw EmulatorKitError.checksumMismatch(expected: sha1, actual: actual, file: destination.lastPathComponent)
            }
        }
    ...
    private func fetch(_ url: URL, part: URL, expectedSize: Int64?,
        ...
        guard let http = response as? HTTPURLResponse else { throw EmulatorKitError.download("no HTTP response") }
        var received: Int64
        ...
        default:
            throw EmulatorKitError.download("HTTP \(http.statusCode) for \(url.lastPathComponent)")
        }
