...
/// script bundle that opens `mandroid://launch/<package>`; its icon is
...
enum LauncherStubBuilder {
    ...
    static let marker = "io.github.madeye.mandroid.stub"
    /// Marker written by the app before it was renamed; such stubs are
    /// still ours (they get rewritten with the new URL scheme).
    static let legacyMarkers = ["io.github.madeye.madroid.stub", "io.github.madeye.androidapprunner.stub"]
    ...
    private static func isOurs(_ bundle: URL) -> Bool {
        ...
        return plist[marker] != nil || legacyMarkers.contains { plist[$0] != nil }
    }
    ...
    private static func write(_ bundle: URL, app: AppInfo) throws {
        ...
        let script = """
        #!/bin/sh
        exec /usr/bin/open \(ADBClient.shellQuote("mandroid://launch/" + app.package.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed)!))
        ...
        let bundleID = "io.github.madeye.mandroid.stub." + app.package
