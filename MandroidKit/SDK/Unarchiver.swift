...
public enum Unarchiver {
    public static func extract(zip: URL, into destination: URL) async throws {
        ...
        guard r.status == 0 else {
            throw MandroidKitError.unarchive("ditto exited \(r.status): \(r.stderrText.trimmingCharacters(in: .whitespacesAndNewlines))")
        }
    ...
    public static func topLevelEntries(zip: URL) async throws -> [String] {
        ...
        guard r.status == 0 else { throw MandroidKitError.unarchive("zipinfo exited \(r.status)") }
