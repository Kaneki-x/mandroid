...
public actor Downloader {
    ...
    public func download(
        ...
        if let sha1 {
            ...
            guard actual == sha1 else {
                ...
                throw MandroidKitError.checksumMismatch(expected: sha1, actual: actual, file: destination.lastPathComponent)
            }
        }
    ...
    private func fetch(_ url: URL, part: URL, expectedSize: Int64?,
        ...
        guard let http = response as? HTTPURLResponse else { throw MandroidKitError.download("no HTTP response") }
        var received: Int64
        ...
        case 206:
            guard let range = http.value(forHTTPHeaderField: "Content-Range"),
                ...
                throw MandroidKitError.download("invalid resume range for \(url.lastPathComponent)")
            }
            rangeTotal = total
        ...
        default:
            throw MandroidKitError.download("HTTP \(http.statusCode) for \(url.lastPathComponent)")
        }
        ...
        if let total, received != total {
            throw MandroidKitError.download("incomplete download: expected \(total) bytes, received \(received)")
        }
