...
public struct RepositoryManifest: Sendable {
    ...
    /// Host architectures that appear in the manifests. Mandroid ships for
    ...
    public static func parse(data: Data, baseURL: URL) throws -> RepositoryManifest {
        ...
        guard parser.parse() else {
            throw MandroidKitError.manifest(parser.parserError?.localizedDescription ?? "invalid XML")
        }
...
private final class ManifestParserDelegate: NSObject, XMLParserDelegate {
    ...
    func parser(_ parser: XMLParser, parseErrorOccurred parseError: Error) {
        error = MandroidKitError.manifest(parseError.localizedDescription)
    }
}
