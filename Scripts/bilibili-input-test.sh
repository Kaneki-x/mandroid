#!/bin/zsh
# Run against a ready Debug Madroid with Bilibili installed and onboarding
# completed. Exercises the empty phone field only; never requests a code.
set -euo pipefail
OUT="${OUT:-$(mktemp -d /tmp/madroid-bili-input.XXXXXX)}"
SCRIPT_DIR="${0:A:h}"
...
grep -q '^state=ready' "$OUT/ready/state.txt" || fail "Madroid is not ready"
...
sleep 3
"$ADB" shell uiautomator dump /data/local/tmp/madroid-input-test.xml >/dev/null
"$ADB" shell cat /data/local/tmp/madroid-input-test.xml > "$OUT/before.xml"
...
sleep 1
"$ADB" shell uiautomator dump /data/local/tmp/madroid-input-test.xml >/dev/null
"$ADB" shell cat /data/local/tmp/madroid-input-test.xml > "$OUT/typed.xml"
...
sleep 1
"$ADB" shell uiautomator dump /data/local/tmp/madroid-input-test.xml >/dev/null
"$ADB" shell cat /data/local/tmp/madroid-input-test.xml > "$OUT/cleared.xml"
